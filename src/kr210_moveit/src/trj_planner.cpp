...
private:
        ...
        std::shared_ptr<kr210_moveit::srv::MoverService::Response> response)
    {
        ...
        // after grasp 
        geometry_msgs::msg::Pose after_grasp = request->pick_pose;
        after_grasp.position.z += 0.4;
        after_grasp.position.y -= 0.4;
        if (!plan_to_pose(after_grasp, previous_joint_positions, traj, previous_joint_positions))
            return;
        trajectories.push_back(traj);

        // // 4. Midpoint joints one (asm_pose1)
        // if (!plan_to_joints(request->asm_pose1.arm_joints, previous_joint_positions, traj, previous_joint_positions))
        // return;
        // trajectories.push_back(traj);


        // 4. Pre Place Pose (Move to Target)
        if (!plan_to_pose(request->place_pose, previous_joint_positions, traj, previous_joint_positions))
            return;
        trajectories.push_back(traj);

        // 5. Release pose (Move closer Slightly)
        geometry_msgs::msg::Pose placed_pose = request->place_pose;
        placed_pose.position.y -= 1.0; 
        if (!plan_to_pose(placed_pose, previous_joint_positions, traj, previous_joint_positions))
            return;
        trajectories.push_back(traj);

        // 6. Release pose (Move back Slightly)
        geometry_msgs::msg::Pose after_placed_pose = request->place_pose;
        after_placed_pose.position.y += 0.3; 
        if (!plan_to_pose(after_placed_pose, previous_joint_positions, traj, previous_joint_positions))
            return;
        trajectories.push_back(traj);

        // // 8. Midpoint joints one (asm_pose1)
        // if (!plan_to_joints(request->asm_pose1.arm_joints, previous_joint_positions, traj, previous_joint_positions))
        // return;
        // trajectories.push_back(traj);
